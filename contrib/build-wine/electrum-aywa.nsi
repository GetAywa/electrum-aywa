...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/akhavr/electrum-aywa"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-aywa-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-aywa.ico"
...
Section
  ...
  File /r "dist\electrum-aywa\*.*"
  File "icons\electrum-aywa.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\dash" "DefaultIcon" "$\"$INSTDIR\electrum-aywa.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\dash\shell\open\command" "" "$\"$INSTDIR\electrum-aywa-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-aywa.ico"
