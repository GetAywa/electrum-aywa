...
SYSTEM_CONFIG_PATH = "/etc/electrum-aywa.conf"
...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-aywa directory", path)
        return path
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-aywa.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-aywa.conf into user_config[]."""
    if not path:
        return {}
